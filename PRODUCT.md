...
Telegram bot client for MiMoCode that lets you run and monitor coding tasks on your local machine from Telegram.
...
The app works as a bridge between Telegram and a locally running MiMoCode server:

- You send prompts from Telegram
- The bot forwards them to MiMoCode
- The app listens to MiMoCode SSE events
...
1. The user works on a project locally with MiMoCode (Desktop/TUI).
...
### MiMoCode server management

- Check MiMoCode server status (running / not running)
- Start MiMoCode server from the app (`opencode serve`)
- Stop MiMoCode server from the app
- Optionally monitor and auto-restart a local MiMoCode server

### Project management

- Fetch available projects from MiMoCode API (name + path)
...
- Use MiMoCode-generated session title (based on conversation)

### Task handling

- Send text prompts to MiMoCode
...
- Handle MiMoCode questions with inline options and custom text answers
- Send selected/custom answers back to MiMoCode (`question.reply`)
...
- `/opencode_start` - start local MiMoCode server
- `/opencode_stop` - stop local MiMoCode server
...
Text messages (non-commands) are treated as prompts for MiMoCode only when no blocking interaction is active. Voice/audio messages are transcribed and then sent as prompts when STT is configured. When `/tts` is enabled globally, completed assistant replies also include a generated audio file if TTS is configured.
...
- Uses MiMoCode local model state (`favorite` + `recent`)
...
- [x] MiMoCode server control from Telegram (`/status`, `/opencode_start`, `/opencode_stop`)
- [x] Project and session management from Telegram (`/projects`, `/worktree`, `/sessions`, `/new`)
- [x] Automatic tracking of the current MiMoCode CLI session, including continuing it from Telegram, live updates, and external text input notifications
...
- [x] Image attachments support (send photos/screenshots from Telegram to MiMoCode, including multiple files in one Telegram album)
- [x] PDF attachments support (send documents from Telegram to MiMoCode)
- [x] Text file attachments support (send code/config/log files from Telegram to MiMoCode)
...
- [x] Create new MiMoCode projects directly from Telegram
- [x] `/mcps` command: browse available MCP servers
- [x] Optional local MiMoCode server monitoring with automatic restart
