...
[![npm version](https://img.shields.io/npm/v/@grinev/mimocode-telegram-bot)](https://www.npmjs.com/package/@grinev/mimocode-telegram-bot)
[![CI](https://github.com/grinev/mimocode-telegram-bot/actions/workflows/ci.yml/badge.svg)](https://github.com/grinev/mimocode-telegram-bot/actions/workflows/ci.yml)
...
MiMoCode Telegram Bot is a secure Telegram client for [MiMoCode](https://mimocode.ai) CLI that runs on your local machine.

Run AI coding tasks, monitor progress, switch models, and manage sessions from your phone.

No open ports, no exposed APIs. The bot communicates with your local MiMoCode server and the Telegram Bot API only.

Scheduled tasks support. Turns the bot into a lightweight OpenClaw alternative for MiMoCode users.
...
<p align="center">
  <img src="assets/screencast.gif" width="45%" alt="MiMoCode Telegram Bot screencast" />
</p>

## Features

- **Remote coding** — send prompts to MiMoCode from anywhere, receive complete results with code sent as files
- **Session management** — create new sessions or continue existing ones, just like in the TUI
- **Track live session** — follow a live MiMoCode CLI session; see [Track Existing Session](#track-existing-session)
...
- **Model switching** — pick models from MiMoCode favorites and recent history directly in the chat (favorites are shown first)
...
- **Custom Commands** — run MiMoCode custom commands (and built-ins like `init`/`review`) from an inline menu with confirmation
- **Skills Catalog** — browse MiMoCode skills from an inline menu and run them immediately or with arguments in the next message
...
- **File attachments** — send images, PDF documents, and text-based files to MiMoCode, including multiple files in one Telegram album
...
- **MiMoCode** — install from [mimocode.ai](https://mimocode.ai) or [GitHub](https://github.com/sst/opencode)
...
### 2. Start MiMoCode Server

Run the MiMoCode server on the same machine where the bot runs:

```bash
opencode serve
```

> The bot connects to the local MiMoCode API at `http://localhost:4096` by default.

> After the bot is configured, you can also start and stop the local MiMoCode server from Telegram with `/opencode_start` and `/opencode_stop`.
...
```bash
npx @grinev/mimocode-telegram-bot@latest
```

> **Note:** This README tracks the `main` branch, which may include unreleased changes. The latest npm release may not include every feature described here yet. See [recent commits on `main`](https://github.com/grinev/mimocode-telegram-bot/commits/main).

> Quick start is for npm usage. You do not need to clone this repository. If you run this command from the source directory (repository root), it may fail with `mimocode-telegram: not found`. To run from sources, use the [Development](#development) section.

On first launch, an interactive wizard will guide you through the configuration — it asks for interface language first, then your bot token, user ID, MiMoCode API URL, and optional MiMoCode server credentials (username/password). After that, you're ready to go. Open your bot in Telegram and start sending tasks.

#### Alternative: Global Install

```bash
npm install -g @grinev/mimocode-telegram-bot
mimocode-telegram start
```
...
```bash
mimocode-telegram start --daemon
mimocode-telegram status
mimocode-telegram stop
```

> Built-in daemon mode is intended for standalone npm installs without an external supervisor. For `systemd`, `pm2`, or Docker, keep using `mimocode-telegram start` without `--daemon`.
...
| `/projects`       | Switch between MiMoCode projects                        |
...
| `/skills`         | Browse and run MiMoCode skills                          |
| `/mcps`           | Browse and toggle MCP servers                           |
| `/task`           | Create a scheduled task                                 |
| `/tasklist`       | Browse and delete scheduled tasks                       |
| `/opencode_start` | Start the local MiMoCode server on the bot machine      |
| `/opencode_stop`  | Stop the local MiMoCode server on the bot machine       |
...
Scheduled tasks let you prepare prompts in advance and run them automatically later or on a recurring schedule. This is useful for periodic checks, routine code maintenance, or tasks you want MiMoCode to execute while you are away from your computer. Use `/task` to create a scheduled task and `/tasklist` to review or delete existing ones.

- Each task is created from the currently selected MiMoCode project and model
...
After you create a new session, select an existing one, or let the bot auto-create one from your first prompt, the bot automatically starts tracking that session. It follows live events from the same MiMoCode CLI session, shows external text input sent from another TUI client, and lets you continue the same session from Telegram.

For this to work, the console MiMoCode instance must be started on the same port the bot connects to. By default, MiMoCode starts on a random port, so use one of the setups below.

- **Single TUI, simplest setup** — start MiMoCode on a fixed port: `opencode --port 4096`
...
- **macOS:** `~/Library/Application Support/mimocode-telegram-bot/.env`
- **Windows:** `%APPDATA%\mimocode-telegram-bot\.env`
- **Linux:** `~/.config/mimocode-telegram-bot/.env`
...
| `OPENCODE_API_URL`                         | MiMoCode server URL                                                                                                   |    No    | `http://localhost:4096`  |
| `OPENCODE_AUTO_RESTART_ENABLED`            | Automatically restart a local MiMoCode server when health-checks fail                                                 |    No    | `false`                  |
| `OPENCODE_MONITOR_INTERVAL_SEC`            | Health monitor interval in seconds when MiMoCode auto-restart is enabled                                              |    No    | `300`                    |
...
4. Send the recognized text to MiMoCode as a normal prompt
...
The model picker uses MiMoCode local model state (`favorite` + `recent`):
...
To add a model to favorites, open MiMoCode TUI (`opencode`), go to model selection, and press **Cmd+F/Ctrl+F** on the model.
...
Since the bot runs locally on your machine and connects to your local MiMoCode server, there is no external attack surface beyond the Telegram Bot API itself.
...
```bash
git clone https://github.com/grinev/mimocode-telegram-bot.git
...
**"MiMoCode server is not available"**

- Ensure an MiMoCode server is running at the configured `OPENCODE_API_URL` (default: `http://localhost:4096`)
- For a local setup, you can start it with `opencode serve` or use `/opencode_start` in Telegram
- For VPS/systemd setups with scheduled tasks, enable `OPENCODE_AUTO_RESTART_ENABLED=true` to let the bot restart a local MiMoCode server when health-checks fail
...
- Add models to your MiMoCode favorites: open MiMoCode TUI, go to model selection, press **Ctrl+F** on desired models
- Verify `OPENCODE_MODEL_PROVIDER` and `OPENCODE_MODEL_ID` point to an available model in your setup

**Linux: permission denied errors**

- Make sure the CLI binary has execute permission: `chmod +x $(which mimocode-telegram)`
- Check that the config directory is writable: `~/.config/mimocode-telegram-bot/`
...
Have questions, want to share your experience using the bot, or have an idea for a feature? Join the [Telegram group](https://t.me/+Fj_IyKRi6-41MGUy) for announcements and discussions, or start a thread in [GitHub Discussions](https://github.com/grinev/mimocode-telegram-bot/discussions).
