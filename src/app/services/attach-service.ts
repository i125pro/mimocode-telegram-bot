...
async function restorePendingQuestion(
  ...
  if (error || !data) {
    if (isExpectedOpencodeUnavailableError(error)) {
      logger.warn("[Attach] MiMoCode server unavailable; skipping pending question restore");
    } else {
...
async function restorePendingPermissions(
  ...
  if (error || !data) {
    if (isExpectedOpencodeUnavailableError(error)) {
      logger.warn("[Attach] MiMoCode server unavailable; skipping pending permission restore");
    } else {
...
export async function attachToSession(deps: AttachSessionDeps): Promise<AttachSessionResult> {
  ...
  if (statusesError) {
    if (isExpectedOpencodeUnavailableError(statusesError)) {
      logger.warn("[Attach] MiMoCode server unavailable; skipping session status restore");
    } else {
...
export async function restoreAttachedCurrentSession(
  ...
  try {
    if (!(await isOpencodeServerHealthy())) {
      logger.warn(
        `[Attach] MiMoCode server is unavailable; skipping followed session restore: session=${currentSession.id}, directory=${currentSession.directory}`,
      );
      return false;
    }
