...
function logModelCatalogRefreshFailure(error: unknown, type: "error" | "exception"): void {
  if (isServerUnavailableError(error)) {
    logger.warn("[ModelManager] MiMoCode server is not running; skipping model catalog refresh");
    return;
  }
...
async function getValidModelKeys(options?: { force?: boolean }): Promise<Set<string> | null> {
  ...
  modelCatalogFetchInFlight = (async () => {
    try {
      logger.debug("[ModelManager] Refreshing model catalog from MiMoCode API");
...
/**
 * Get favorite and recent models from MiMoCode local state file.
...
export async function getModelSelectionLists(): Promise<ModelSelectionLists> {
  ...
  try {
    ...
    if (filteredOutFavorites > 0 || filteredOutRecent > 0) {
      logger.info(
        `[ModelManager] Filtered unavailable models from MiMoCode state: favoritesRemoved=${filteredOutFavorites}, recentRemoved=${filteredOutRecent}`,
      );
    }
    ...
    if (envDefaultModel) {
      logger.warn(
        "[ModelManager] Failed to load MiMoCode model state, using config model as favorite:",
        err,
      );
      return {
    ...
    logger.error("[ModelManager] Failed to load MiMoCode model state:", err);
    return {
      favorites: [],
      recent: [],
    };
...
/**
 * Validate stored selected model against MiMoCode providers catalog.
...
/**
 * Get list of favorite models from MiMoCode local state file
