...
class PinnedMessageManager {
    ...
    try {
      ...
      if (error || !messagesData) {
        if (isExpectedOpencodeUnavailableError(error)) {
          logger.warn("[PinnedManager] MiMoCode server unavailable; skipping session history load");
        } else {
  ...
  private async loadDiffsFromApi(sessionId: string): Promise<void> {
    try {
      ...
      if (isExpectedOpencodeUnavailableError(err)) {
        logger.debug("[PinnedManager] MiMoCode server unavailable; skipping diff restore");
      } else {
  ...
  private async loadDiffsFromMessages(sessionId: string, directory: string): Promise<void> {
    try {
      ...
      if (error || !messagesData) {
        if (isExpectedOpencodeUnavailableError(error)) {
          logger.debug("[PinnedManager] MiMoCode server unavailable; skipping diff message restore");
        } else {
  ...
  private async refreshSessionTitle(): Promise<void> {
    ...
    try {
      ...
      if (isExpectedOpencodeUnavailableError(err)) {
        logger.debug("[PinnedManager] MiMoCode server unavailable; skipping session title refresh");
      } else {
  ...
  private async fetchContextLimit(): Promise<void> {
    try {
      ...
      if (isExpectedOpencodeUnavailableError(err)) {
        logger.warn("[PinnedManager] MiMoCode server unavailable; using default context limit");
      } else {
