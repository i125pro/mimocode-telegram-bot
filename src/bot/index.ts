...
export function createBot(): Bot<Context> {
  ...
  unsubscribeReadyRestore = opencodeReadyLifecycle.onReady(async (reason) => {
    ...
    if (restored) {
      logger.info(`[Bot] Restored followed session after MiMoCode ready: reason=${reason}`);
      return;
    }
    ...
    if (config.bot.trackBackgroundSessions && currentProject?.worktree) {
      ...
      logger.info(
        `[Bot] Started background session tracking after MiMoCode ready: reason=${reason}, directory=${currentProject.worktree}`,
      );
    }
  });
