...
async function getHealthIfAvailable(): Promise<HealthCheckResult | null> {
  try {
    ...
    if (result === HEALTH_CHECK_TIMED_OUT) {
      logger.warn(`[Bot] MiMoCode health check timed out after ${HEALTH_CHECK_TIMEOUT_MS}ms`);
      return null;
    }
...
/**
 * Wait for MiMoCode server to become ready by polling health endpoint
 ...
 * Starts the MiMoCode server process
 */
export async function opencodeStartCommand(ctx: CommandContext<Context>) {
  try {
    ...
    childProcess.once("error", (error) => {
      logger.error("[Bot] MiMoCode server process failed to start", error);
    });
    ...
    logger.info("[Bot] Waiting for MiMoCode server to become ready...");
    ...
    logger.info(`[Bot] MiMoCode server started successfully, PID=${pid}, port=${localTarget.port}`);
