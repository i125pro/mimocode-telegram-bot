...
export const zh: I18nDictionary = {
  ...
  "cmd.description.mimocode_start": "启动 MiMoCode 服务器",
  "cmd.description.mimocode_stop": "停止 MiMoCode 服务器",
  ...
  "bot.prompt_send_error": "向 MiMoCode 发送请求失败。",
  "bot.session_error": "🔴 MiMoCode 返回错误：{message}",
  ...
  "bot.media_group_not_processed": "⚠️ 此相册中有一个或多个文件无法处理。未向 MiMoCode 发送任何内容。",
  "bot.media_group_download_error": "🔴 无法下载其中一个文件。未向 MiMoCode 发送任何内容。",
  "bot.model_no_pdf": "⚠️ 当前模型不支持PDF输入。将仅发送文本。",
  "bot.text_file_too_large": "⚠️ 文本文件过大（最大 {maxSizeKb}KB）",

  "status.header_running": "🟢 MiMoCode 服务器正在运行",
  ...
  "status.server_unavailable": "🔴 MiMoCode 服务器不可用\n\n使用 /mimocode_start 启动服务器。",
  ...
  "projects.empty":
    "📭 未找到项目。\n\n在 MiMoCode 中打开一个目录并至少创建一个会话，然后它会出现在这里。",
  ...
  "projects.fetch_error": "🔴 MiMoCode 服务器不可用，或加载项目时发生错误。",
  ...
  "sessions.fetch_error": "🔴 MiMoCode 服务器不可用，或加载会话时发生错误。",
  ...
  "messages.fetch_error": "🔴 MiMoCode Server 不可用，或加载消息时发生错误。",
  ...
  "detach.success":
    "✅ 已从会话分离：{title}\n\nMiMoCode 会话未停止。如果它仍在运行，将继续独立执行。稍后可通过 /sessions 重新选择它来查看。",
  "detach.error": "🔴 无法从当前会话分离。",

  "new.project_not_selected": "🏗 未选择项目。\n\n请先使用 /projects 选择一个项目。",
  "new.created": "✅ 新会话已创建：{title}",
  "new.create_error": "🔴 MiMoCode 服务器不可用，或创建会话时发生错误。",
  ...
  "mimocode_start.already_running_external":
    "✅ MiMoCode 服务器正作为外部进程运行\n\n版本：{version}\n\n该服务器不是由机器人启动，因此 /mimocode-stop 无法停止它。",
  "mimocode_start.already_running": "✅ MiMoCode 服务器已在运行\n\n版本：{version}",
  "mimocode_start.remote_configured": "⚠️ /mimocode_start 仅适用于本地 MiMoCode 服务器。",
  "mimocode_start.starting": "🔄 正在启动 MiMoCode 服务器...",
  "mimocode_start.start_error":
    "🔴 启动 MiMoCode 服务器失败\n\n错误：{error}\n\n请检查 MiMoCode CLI 已安装且在 PATH 中可用：\nmimocode --version\nnpm install -g @mimocode-ai/cli",
  "mimocode_start.started_not_ready":
    "⚠️ MiMoCode 服务器已启动，但未响应\n\nPID：{pid}\n\n服务器可能仍在启动中。几秒后试试 /status。",
  "mimocode_start.success": "✅ MiMoCode 服务器启动成功\n\nPID：{pid}\n版本：{version}",
  "mimocode_start.error": "🔴 启动服务器时发生错误。\n\n请查看应用日志了解详情。",
  "mimocode_stop.external_running":
    "⚠️ MiMoCode 服务器正作为外部进程运行\n\n该服务器不是通过 /mimocode-start 启动的。\n请手动停止它，或使用 /status 检查状态。",
  "mimocode_stop.remote_configured": "⚠️ /mimocode_stop 仅适用于本地 MiMoCode 服务器。",
  "mimocode_stop.not_running": "⚠️ MiMoCode 服务器未运行",
  "mimocode_stop.pid_not_found":
    "⚠️ MiMoCode 服务器正在端口 {port} 上响应，但未找到可停止的本地进程。",
  "mimocode_stop.stopping": "🛑 正在停止 MiMoCode 服务器...\n\nPID：{pid}",
  "mimocode_stop.stop_error": "🔴 停止 MiMoCode 服务器失败\n\n错误：{error}",
  "mimocode_stop.still_running": "停止请求后服务器仍在响应。",
  "mimocode_stop.success": "✅ MiMoCode 服务器已成功停止",
  "mimocode_stop.error": "🔴 停止服务器时发生错误。\n\n请查看应用日志了解详情。",
  ...
  "model.menu.favorites_title": "⭐ 收藏（可在 MiMoCode CLI 中将模型加入收藏）",
  "model.menu.favorites_empty": "— 列表为空。",
  "model.menu.recent_title": "🕘 最近使用",
  "model.menu.recent_empty": "— 列表为空。",
  "model.menu.favorites_hint": "ℹ️ 可在 MiMoCode CLI 中将模型加入收藏，使其显示在列表顶部。",
  ...
  "commands.select": "请选择一个 MiMoCode 命令：",
  "commands.empty": "📭 当前项目没有可用的 MiMoCode 命令。",
  "commands.fetch_error": "🔴 加载 MiMoCode 命令失败。",
  ...
  "commands.execute_error": "🔴 执行 MiMoCode 命令失败。",
  "commands.select_page": "请选择一个 MiMoCode 命令（第 {page} 页）：",
  ...
  "skills.select": "请选择一个 MiMoCode 技能：",
  "skills.empty": "📭 当前项目没有可用的 MiMoCode 技能。",
  "skills.fetch_error": "🔴 加载 MiMoCode 技能失败。",
  ...
  "skills.select_page": "请选择一个 MiMoCode 技能（第 {page} 页）：",
  ...
  "legacy.models.empty": "📋 没有可用模型。请在 MiMoCode 中配置 providers。",
