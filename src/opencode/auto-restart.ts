...
export class OpencodeAutoRestartService {
  ...
  private async checkAndRestart(reason: "startup" | "interval"): Promise<void> {
    ...
    try {
      ...
      logger.warn(
        `[OpenCodeAutoRestart] MiMoCode server is unavailable, starting local server: reason=${reason}, port=${this.localTarget.port}`,
      );

      const childProcess = startLocalOpencodeServer(this.localTarget);
      childProcess.once("error", (error) => {
        logger.error("[OpenCodeAutoRestart] MiMoCode server process failed to start", error);
      });
      ...
      if (!ready) {
        logger.warn(
          `[OpenCodeAutoRestart] MiMoCode server was started but did not become ready: pid=${pid ?? "unknown"}, port=${this.localTarget.port}`,
        );
        return;
      }

      logger.info(
        `[OpenCodeAutoRestart] MiMoCode server recovered: pid=${pid ?? "unknown"}, port=${this.localTarget.port}`,
      );
      this.serverWasHealthy = true;
      await opencodeReadyLifecycle.notifyReady(`auto_restart_${reason}`);
    } catch (error) {
      logger.error("[OpenCodeAutoRestart] Failed to check or restart MiMoCode server", error);
    } finally {
